...
public class AllSearchResultFragment extends FragmentController {
    ...
    public AllSearchResultFragment(List<HomeContentPOJO> podcastPOJOS, List<PodcastEpisodeDetailsPOJO> podcastEpisodeDetailsPOJOS, List<HomeContentPOJO> artistePojos, List<HomeContentPOJO> radioPOJOS) {
        if (podcastPOJOS.size() > 4) {
            for (int i = 0; i < podcastPOJOS.size(); i++) {
                this.podcastPOJOS.add(podcastPOJOS.get(i));
            }
        } else {
            this.podcastPOJOS.addAll(podcastPOJOS);
        }

        if (podcastEpisodeDetailsPOJOS.size() > 4) {
            for (int i = 0; i < podcastEpisodeDetailsPOJOS.size(); i++) {
                this.podcastEpisodeDetailsPOJOS.add(podcastEpisodeDetailsPOJOS.get(i));
            }
        } else {
            this.podcastEpisodeDetailsPOJOS.addAll(podcastEpisodeDetailsPOJOS);
        }

        if (artistePojos.size() > 4) {
            for (int i = 0; i < artistePojos.size(); i++) {
                this.artistePojos.add(artistePojos.get(i));
            }
        } else {
            this.artistePojos.addAll(artistePojos);
        }

        if (radioPOJOS.size() > 4) {
            for (int i = 0; i < radioPOJOS.size(); i++) {
                this.radioPOJOS.add(radioPOJOS.get(i));
            }
        } else {
            this.radioPOJOS.addAll(radioPOJOS);
        }
    }
...
//            searchRadioAdapter.notifyDataSetChanged();
    ...
    public void setAllSearchAdapter(List<HomeContentPOJO> podcastPOJOS, List<PodcastEpisodeDetailsPOJO> podcastEpisodeDetailsPOJOS, List<HomeContentPOJO> artistePojos, List<HomeContentPOJO> radioPOJOS) {
        try {
            this.podcastPOJOS.clear();
            this.podcastPOJOS.addAll(podcastPOJOS);
            if (podcastPOJOS.size() > 0) {
                ll_podcasts.setVisibility(View.VISIBLE);
            } else {
